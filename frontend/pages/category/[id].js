...
function Index(props) {
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      if (id === null) {
        props.getBlogsByCategoryAction(localStorage.getItem("currentCategory"));
      } else {
        props.getBlogsByCategoryAction(id);
      }
    }
  }, []);
  console.log(id);
        ...
        rows.push(
          <Link href={href}>
            <a
              className="list-item"
              onClick={() => {
                props.getBlogsByCategoryAction(i.id);
              }}
            >
              {i.name}
            </a>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  getBlogsByCategoryAction: bindActionCreators(getBlogsByCategory, dispatch),
});
const mapStateToProps = (state) => ({
  categories: state.categoriesReducers.categories,
  blogs: state.categoriesReducers.blogs,
});
